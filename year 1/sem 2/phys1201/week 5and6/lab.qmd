...
The experiment was analysed using Python. I used `numpy` for numerical calculations, `matplotlib` for graphs, `scipy` for statistical tests and ODR, and `uncertainties` for propagating measured uncertainties.

Uncertainties are as given below:

```{python}
sigma_N_turns = 1          # miscounting a partial turn, etc.
sigma_R_cm = 0.1           # ruler on calculating R
sigma_h_cm = 0.2           # ruler reading on drop height
sigma_emf = 0.02      # 2% of oscilloscope voltage reading
```
...
def compare_to_theory(x_vals, y_measured_v, y_theory, title):
    ...
    sigma_y = y_measured_v * sigma_emf
    y_nom = y_measured_v
...
The uncertainties of measured quants are as follows:

| Quantity | Uncertainty |
|---|---:|
| Number of turns \(N\) | \(1\) turn |
| Coil radius \(R\) | \(0.1\) cm |
| Drop height \(h\) | \(0.2\) cm |
...
The individual (k)-values had a spread of approximately 27% relative to their mean. The largest deviations occurred at the closest measurement points, where the measured field showed increasing deviation from the ideal $z^{-3}$ dependence.
...
Independent best-fit models were applied to each dataset to distinguish differences in scale or offset from differences in the predicted functional dependence.

For the coil-radius data, a power-law model of the form $\varepsilon=aR^b$ was used. The fitted exponent was approximately (1.53), compared with the theoretical value of (-2). This represents the clearest deviation in the functional form of the experimental results and is consistent with limitations of the point-dipole approximation when the coil radius becomes comparable to the magnet's dimensions.
...
All three datasets had (R^2) values above 0.94, indicating that the measurements were internally consistent and exhibited relatively little scatter. The (N) and (s) datasets showed particularly strong agreement with their predicted functional forms, while the radius data showed the main deviation in shape.
...
The agreement was not perfect, mainly because the theoretical model assumes an ideal point dipole, ideal free fall, perfectly centred motion and an ideal circular coil. Despite these limitations, the experimental trends were consistent with the main predictions of Faraday's law and provided a reasonable demonstration of electromagnetic induction.
