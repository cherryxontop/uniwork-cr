...
def odr_fit(x: list[ufloat], y: list[ufloat], zero_intercept: bool = False) -> odr_fit_result:
    ...
    if zero_intercept:
        model = odr.Model(lambda p, x: p[0] * x)
        beta0 = [10.0]
    else:
    ...
    intercept = ufloat(0, 0) if zero_intercept else ufloat(odr_result.beta[1], odr_result.sd_beta[1])

    y_pred = model.fcn(odr_result.beta, x_nom)
    residuals = y_nom - y_pred
    ss_res = np.sum(residuals**2)

    ss_tot = np.sum(y_nom**2) if zero_intercept else np.sum((y_nom - np.mean(y_nom))**2)
    r_squared = 1 - ss_res / ss_tot if ss_tot != 0 else np.nan

    rmse = np.sqrt(np.mean(residuals**2))

    num_params = 1 if zero_intercept else 2
    dof = len(x_nom) - num_params
    chi_squared_reduced = np.sum((residuals / y_err) ** 2) / dof if dof > 0 else np.nan
...
def plot(result: odr_fit_result, xlabel, ylabel, title, filename):
    x, y, sx, sy = result.x_nom, result.y_nom, result.x_err, result.y_err
    slope, intercept = result.slope.n, result.intercept.n
    ...
    plt.plot(
        x_fit, y_fit,
        "k--",
        label=f"Fit: Slope = {result.slope.n:.3f} ± {result.slope.s:.3f}"
    )
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
for i in range(len(angles_deg)):
    ...
    # Conditional angle correction factor (only applied if angle >= 10°)
    if theta_deg >= 10.0:
...
lengths_p2_cm_trials = [20.0, 40.0, 60.0, 80.0]
time_10_p2 = [
    [9.33, 9.33, 9.41],
    [13.08, 13.08, 13.01],
    [15.68, 15.63, 15.80],
...
lengths_m = [ufloat(l / 100.0, sigma_L) for l in lengths_p2_cm_trials]
...
print(f"reduced chi^2   : {result.chi_squared_reduced:.4f}")
...
plot(
    ...
    filename="/Users/chhaya/Documents/uniwork-cr/year 1/sem 2/phys1201/week 3/graphs/pendulum_length_dependence.png"
)